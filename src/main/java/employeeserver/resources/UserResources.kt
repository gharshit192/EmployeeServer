...
class UserResource {
    ...
    fun addUser(request: String): Response {
        ...
        return if (!user) {
            ...
            Response.ok(user.toString()).entity("User added successfully").build()
        }
    }
    ...
    fun getUser(@QueryParam("id") id: String): Response {
        ...
        return if (!getUser){
            Response.ok().entity("User not find").build()
        }else{
            Response.ok(getUser).entity("User find successfully").build()
        }
    }
